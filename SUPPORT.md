...
**`keybindings.json`** -- A keymap file, joined against the agent directory beside the owned `settings.json`. Not owned, for the reason claude's row gives: no component kind describes a keymap. ([source](measured from the 0.84.3 bundle, package/dist/core))
...
**`pi-debug.log`** -- Written by the hidden `/debug` command and holding rendered TUI lines with ANSI codes. A log, never owned and never captured. ([source](measured 2026-08-28 in the product's own shipped documentation, read from the 0.84.3 package at node_modules/@earendil-works/pi-coding-agent/docs))
...
**`working-directories`** -- One row for `git`, `npm` and `tmp`, joined against the agent directory. Scratch space the product manages for its own operations. ([source](measured from the 0.84.3 bundle, package/dist/core))
