...
fn through_remove(exe: &Path, target: &Path, own: &[String], first: &str, found: &mut Vec<String>) {
    ...
    //
    // Composition withdraws recorded files. The human surface must say that,
    // and name the restore command. Checked here because those words exist on
    // the printed surface.
    for wanted in [
        "only the files this provider recorded writing",
        "restore it with",
    ] {
